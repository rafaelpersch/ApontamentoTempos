  ...
  <div>
      ...
      <div class="col-md-12">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Tempos</li>
          </ol>
        </nav>
      </div>
    </div>  
    <div class="row">
      <div class="col-md-12 text-right">
        <router-link :to="{ path: '/Principal/Tempo'}">
     ...
     <v-server-table ref="table" 
        ...
        <template slot="id" scope="props">
          <div>
            <router-link class="btn btn-primary mt-2" :to="{ path: '/Principal/Tempo/' + props.row.id }">
...
export default {
    ...
    return {
      ...
      options: {
        ...
        requestFunction: function (data) {
          ...
          return this.httpService.get('api/ApontamentoTempo?query=' + data.query + 
        ...
        responseAdapter : function(resp) {
          ...
          var data2 = [];
          for (var key in data) {
            data2.push({data:data[key].dataDDMMYYYY, projeto:data[key].projeto.nome, issue:data[key].issue, id: data[key].id});
          }          
  ...
  methods: {
    deleteItem(id){
      ...
      this.httpService.delete('api/ApontamentoTempo', id, false).then(resolve => {
        if (resolve.status == 200){
          this.$toast.success({
              title:'Success',
